...
class SettingTabs extends Tabs
{
    ...
    {
        return tap(new Tab('klarna', trans('setting::settings.tabs.klarna')), function (Tab $tab) {
            $tab->weight(60);

            $tab->fields([
                'klarna_enabled',
                'translatable.klarna_label',
                'translatable.klarna_description',
                'klarna_publishable_key',
                'klarna_secret_key',
            ]);

            $tab->view('setting::admin.settings.tabs.klarna');
        });
    }
